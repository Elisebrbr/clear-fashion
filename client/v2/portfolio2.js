...
Description of the available api endpoints :
  ...
  GET https://clear-fashion-gqis-elisebrbr.vercel.app/procucts/:id
  Search with filter :
  GET https://clear-fashion-gqis-elisebrbr.vercel.app/products/search?brand=dedicated&limit=10&price=50
  This endpoint accepts the following optional query string parameters:
  - `brand` - choose a brand
  - `limit` - number of products to return
  - `price`
  Search for available brands list :
  GET https://clear-fashion-gqis-elisebrbr.vercel.app/
*/
...
const fetchProducts = async (page = 1, size = 12) => {
  try {
    const response = await fetch(
        `https://clear-fashion-gqis-b2go0mp67-elisebrbr.vercel.app/products/search?limit=${limit}&brand=${brand}&price=${price}`
    );
...
async function fetchBrands() {
  try {
    const response= await fetch(
        'https://clear-fashion-gqis-b2go0mp67-elisebrbr.vercel.app/brands'
    );
    const body = await response.json();
